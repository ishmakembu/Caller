...
interface Room {
  id: string
  peers: Map<string, { socketId: string; displayName: string }>
...
function createRoom(id: string, hostSocketId: string): Room {
  const room: Room = {
    id,
    peers: new Map(),
    hostSocketId,
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('join', ({ roomId, displayName }: { roomId: string; displayName: string }) => {
  ...
  socket.on('signal:ice-candidate', ({ targetSocketId, candidate }: { targetSocketId: string; candidate: RTCIceCandidateInit }) => {
    const roomId = socket.data.roomId
    if (!roomId) return
  ...
  socket.on('room:kick', ({ socketId }: { socketId: string }) => {
    const roomId = socket.data.roomId
    if (!roomId) return
